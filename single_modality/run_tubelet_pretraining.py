...
from engines.engine_for_pretraining_tubelet import train_one_epoch
...
def main(args):
    ...
    if args.distributed:
        ...
        model = torch.nn.parallel.DataParallel(model, find_unused_parameters=True)
        moco = torch.nn.parallel.DataParallel(moco, find_unused_parameters=True)
        model_without_ddp = model.module
        moco_model_without_ddp = moco.module
        teacher_model = torch.nn.parallel.DataParallel(teacher_model, find_unused_parameters=False)
