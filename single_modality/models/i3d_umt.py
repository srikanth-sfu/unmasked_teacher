...
class InceptionI3d(nn.Module):
    ...
    def __init__(self, num_classes=400, spatial_squeeze=True,
        ...
        for key, input_dim in zip(self.VALID_ENDPOINTS_UMT, self.INPUTS_UMT_DIM):
            self.end_points_umt[key] = Unit3D(in_channels=input_dim, output_channels=512,
                             kernel_shape=[1, 1, 1],
                             padding=0,
                             activation_fn=None,
                             use_batch_norm=False,
    ...
    def transform_umt(self, feats):
        out = []
        for feat in feats:
            n, c, t, h, w = feat.shape
            h1, w1 = 14, 14
            factor = 4 // t
            feat_reshaped = feat.permute(0,2,1,3,4).reshape(n*t, c, h, w)
            feat_resized = F.interpolate(feat_reshaped, size=(h1, w1), mode='bilinear', align_corners=False)
            feat_resized = feat_resized.view(n, c, t, h1, w1).repeat(1,1,factor,1,1).view(n, c, factor*t*h1*w1)
